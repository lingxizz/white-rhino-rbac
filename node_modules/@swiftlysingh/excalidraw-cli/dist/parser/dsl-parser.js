    ...
    while (i < len) {
        ...
        if (input[i] === '@') {
            ...
            if (directive === 'decorate') {
                ...
                const src = parts[0] || '';
                const anchor = (parts[1] || 'top-right');
                tokens.push({
                    type: 'decorate',
                    value: src,
                    imageSrc: src,
 ...
 */
function parseImageDirective(value) {
    // Match: path at X,Y
 ...
 */
function parseScatterDirective(value) {
    const parts = value.trim().split(/\s+/);
    if (parts.length < 2)
        return null;
    const src = parts[0];
...
export function parseDSL(input) {
    ...
    function getOrCreateNode(label, type, imageData) {
