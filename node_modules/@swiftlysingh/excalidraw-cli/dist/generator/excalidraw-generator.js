...
import { createNode, createArrow, createNodeLabel, createEdgeLabel, resetIndexCounter, createImageElement, createPositionedImageElement, createFileData, generateFileId, getImageDimensions, } from '../factory/index.js';
 ...
 */
function getDecorationOffset(anchor, nodeWidth, nodeHeight, imageWidth, imageHeight) {
    const margin = 5;
    switch (anchor) {
        case 'top':
 ...
 */
function generateScatteredImages(scatter, canvasWidth, canvasHeight, elements, files, libraryPath) {
    for (const config of scatter) {
        ...
        for (let i = 0; i < config.count; i++) {
            ...
            if (fileData) {
                ...
                const imageElement = createPositionedImageElement({ id: imageId, src: config.src, x, y, width, height }, fileId);
...
export function generateExcalidraw(graph) {
        ...
        if (node.type === 'image' && node.image) {
            ...
            if (node.decorations) {
                for (const decoration of node.decorations) {
                    ...
                    const offset = getDecorationOffset(decoration.anchor, node.width, node.height, dims.width, dims.height);
                    const fileId = generateFileId();
    ...
    if (graph.scatter && graph.scatter.length > 0) {
        generateScatteredImages(graph.scatter, graph.width, graph.height, elements, files, graph.library);
    }
    return {
        type: 'excalidraw',
        version: 2,
